...
import com.example.chapterly.data.cloud.NoOpCloudSyncDataSource
...
import com.example.chapterly.data.remote.ImageLinksDto
import com.example.chapterly.data.remote.VolumeDto
import com.example.chapterly.data.remote.VolumeInfoDto
import com.example.chapterly.data.remote.VolumesResponse
...
class OfflineFirstBookRepositoryTest {
    @get:Rule
    ...
    private val repository: OfflineFirstBookRepository by lazy {
        OfflineFirstBookRepository(
            api = fakeApi,
            database = database,
            bookDao = bookDao,
            cloudSyncDataSource = NoOpCloudSyncDataSource(),
        )
    }
    ...
    @Test
    fun pagedBooks_fetchesAndCachesFromNetwork() =
        runTest {
            ...
            assertNotNull(bookDao.observeBook("android-0").first())
        }

    @Test
    fun favoritesFilter_returnsOnlyFavoritedBooks() =
        runTest {
    ...
    @Test
    fun favoritesFilter_keepsFavoritesAcrossDifferentSearchQueries() =
        runTest {
            fakeApi.seedQuery("android", count = 15)
            fakeApi.seedQuery("kotlin", count = 15)
            ...
            val bookToFavorite = kotlinResults.first()
            ...
            assertTrue(favoritesOnly.any { it.id == bookToFavorite.id })
        }

    @Test
    fun saveSearchQuery_updatesHistory() =
        runTest {
